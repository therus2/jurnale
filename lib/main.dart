...
class _HomePageState extends State<HomePage> {
  ...
  // ======= Загрузка замен =======
  Future<void> _openReplacementsInBrowser(BuildContext context) async {
    ...
    try {
      if (await canLaunchUrl(url)) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Не удалось открыть в браузере')),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
